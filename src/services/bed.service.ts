...
export class BedService {
    ...
    async create(roomId: string, label: string, rentAmount: number, propertyId: string, isOccupied: boolean) {
        // Ensure Room exists
        await this.roomService.getById(roomId, propertyId);

        const bed = await this.bedRepository.create({ roomId, label, rentAmount, propertyId, isOccupied });
