...
import AnsiColour, { AnsiReset } from "./enums/log-colours.ts";
import { env } from "../config/env-validator.ts";
import type { RequestWithBodyContent, ResponseWithBodyContent } from "../middlewares/logger.ts";
import {RedirectionStatus, SuccessStatus, ClientErrorStatus, ServerErrorStatus} from "./enums/http.ts";

type LogSeverity = "DEBUG" | "INFO" | "WARN" | "ERROR" | "CRITICAL";
type LogMode = "server" | "user" | "api";

type LogLabels<M extends LogMode> = M extends "user" ? { username: string; } : undefined;
