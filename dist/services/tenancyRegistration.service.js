...
export class TenancyRegistrationService {
    ...
    async register(userId, request) {
        return await this.unitOfWork.execute(async (repositories) => {
            const { guestRepository, tenancyRepository, propertyRepository, roomRepository, bedRepository, } = repositories;
            // 1. Guest must exist
            ...
            const activeTenancy = await tenancyRepository.getActiveTenancyByGuestId(guest.guestId);
            if (activeTenancy) {
            ...
            // 5. Bed must exist
            const bed = await bedRepository.findById(request.bedId, request.propertyId, request.roomId);
            if (!bed) {
            ...
            // 9. Occupy the bed
            await bedRepository.setOccupied(request.bedId, request.propertyId, request.roomId, true);
            // 10. Increment room occupancy
            await roomRepository.incrementOccupiedCount(request.propertyId, request.roomId, 1);
            return tenancy;
        });
    }
    async getActiveTenancy(userId) {
        return await this.unitOfWork.execute(async (repositories) => {
    ...
    async getTenanciesByProperty(propertyId) {
        return await this.unitOfWork.execute(async (repositories) => {
    ...
    //   });
    // }
    async getGuestsByProperty(propertyId) {
        return await this.unitOfWork.execute(async (repositories) => {
            const tenancies = await repositories.tenancyRepository.getActiveTenanciesByPropertyId(propertyId);
            const guestIds = tenancies.map((t) => t.guestId);
            const guests = await repositories.guestRepository.getGuestsByIds(guestIds);
...
//# sourceMappingURL=tenancyRegistration.service.js.map